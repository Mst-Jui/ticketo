...
import { FaHistory, FaHome, FaSignOutAlt, FaTicketAlt, FaUserCircle, FaUserShield } from 'react-icons/fa';
import { FaBuilding, FaCalendarAlt, FaPlus, FaUsers } from "react-icons/fa";
          ...
          <nav className="flex-grow overflow-y-auto px-3 py-4 space-y-1">
            ...
            {menuItems?.map(({ key, label, icon: Icon, href }) => {
              return (
                ...
                >
                  <span className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-color bg-white/5 text-slate-400"}`}>
                    <Icon size={14} />
                  </span>
                  <span>{label}</span>
